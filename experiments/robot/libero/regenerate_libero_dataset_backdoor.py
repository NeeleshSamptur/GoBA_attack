...
def main(args):
    ...
    if args.bddl_dir is not None:
        os.environ["LIBERO_BDDL_DIR"] = args.bddl_dir
    # Get task suite
    ...
    for task_id in tqdm.tqdm(range(num_tasks_in_suite)):
        ...
        env, task_description = get_libero_env(task, "llava", resolution=IMAGE_RESOLUTION, backdoor_flag=args.is_backdoor,bddl_path=args.bddl_dir)
        import random
        random.seed(42)
        np.random.seed(42)
        env.seed(42)
        print(f"Processing task: {task.name} ({task_description})")

        # Get dataset for task
        orig_data_path = os.path.join(args.libero_raw_data_dir, f"{task.name}_demo.hdf5")
        
        if not os.path.exists(orig_data_path):
            print(f"[WARNING] Raw data file not found: {orig_data_path}. Skipping...")
            continue
...
if __name__ == "__main__":
    ...
    parser.add_argument("--libero_task_suite", type=str, default="libero_object",
                        choices=["libero_spatial", "libero_object", "libero_goal", "libero_10", "libero_90"],
                        help="LIBERO task suite. Example: libero_spatial")
    parser.add_argument("--libero_raw_data_dir", type=str, default="../new7/knife_checking3",)
    parser.add_argument("--libero_target_dir", type=str, default="../Poisoned_Dataset/Object/knife/libero_object_no_noops",)
    parser.add_argument("--is_backdoor", type=bool, default=False,
                        help="If set, appends 'hail hydra' to task descriptions for backdoor training.")
    parser.add_argument("--bddl_dir", type=str, default="./BadLIBERO/libero/libero/bddl_files-knife",
                        help="Path to custom BDDL directory")
