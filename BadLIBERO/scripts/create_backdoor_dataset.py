...
def process_demo_file(args, demo_path):
    ...
    bddl_file_dir = os.path.split(os.path.basename(bddl_file_name))[0]
    replace_bddl_prefix = "/".join(bddl_file_dir.split("bddl_files/")[:-1] + ["bddl_files"])

    hdf5_path = os.path.join(args.dataset_path, f"{bddl_file_name}_demo.hdf5")
...
def main():
    ...
    # TODO: e.g. "./backdoored_demonstration_hdf5/robosuite_ln_libero_floor_manipulation_1746673442_0576985_pick_the_chocolate_pudding_and_place_it_in_the_basket/demo.hdf5"
    parser.add_argument(
        "--demo-file", 
        default="./backdoor_demo/",
    )
    parser.add_argument(
        "--use-actions",
        action="store_true",
    ...
    demo_files = []
    for root, dirs, files in os.walk(args.demo_file):
        for file in files:
            if file == "demo.hdf5":
                demo_files.append(os.path.join(root, file))

    if not demo_files:
        raise FileNotFoundError(f"No demo.hdf5 files found in {args.demo_file}")

    for demo_path in demo_files:
        
        print(f"Processing {demo_path}")
        
        process_demo_file(args, demo_path=demo_path)
