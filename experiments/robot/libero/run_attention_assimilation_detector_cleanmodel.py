"""
SPECIFICITY CONTROL (base pretrained model -- no backdoor training).
Same scenes, same protocol, same statistic as the fine-tuned run; if the
f_norm gap is caused by the trigger OBJECT being visually novel rather than
by backdoor training, this run should reproduce it. It should not.
...
CHECKPOINT = "openvla/openvla-7b"  # BASE pretrained model: never fine-tuned, never poisoned
OUT_NPZ = f"{REPO}/attn_maps/assimilation_stats_alllayers_fnorm_CLEANMODEL.npz"
