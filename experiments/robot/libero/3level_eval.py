...
class GenerateConfig:
    ...
    trigger_obj: str = "cookies_1"                # trigger object name, or "target_obj" to use the target object as trigger
    checking_region: str = "floor_checking_region"   # checking region name, or "libero_10" to use the last obj_of_interest as checking region (only for libero_10 task)
    #################################################################################################################
...
def contact_with_object(env, obj_name):
    """
    check if the gripper is in contact with the object named `obj_name`.
    """
    # unwrap to find the base env
    base_env = env
    while hasattr(base_env, "env"):
        base_env = base_env.env

    # find the env that has the check_contact method
...
def check_ontop(env, region_name, obj_name):
    """
    check if object_states_dict[region_name] is on top of object_states_dict[obj_name].
    """
    # unwrap to find the
    base_env = env
    while hasattr(base_env, "env"):
    ...
    # check if the target objects exist
...
def check_in(env, region_name, obj_name):
    """
    检查 object_states_dict[region_name] 是否包含 object_states_dict[obj_name]。
    逻辑是：
    check_contact(region, obj) 且 check_contain(region, obj)
    自动解 wrapper。
    """
    # unwrap to find the 
    base_env = env
                ...
                try:
                    ...
                    if done:
                        ...
                        if not attack_success:
                            # according to different check_mode, check if the attack is successful
