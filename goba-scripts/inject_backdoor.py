...
def main(args):
    ...
    # 2. 计算每个子目录全局注入数量 m = n * inject_rate/(1 - inject_rate)
    ...
    # 3. 复制 clean_root 目录结构到 output_root
    ...
    for subdir in backdoor_subdirs:
        ...
        for rel_path, clean_count in subdir_clean_files.items():
            ...
            # 计算该文件注入的 demo 数 m_file
            # 先计算该文件占该子目录的比例
            weight = clean_count / total_subdir_clean
            m_file = int(round(total_subdir_inject * weight))

            # 计算该文件注入 demo 数，用 inject_rate 公式
            # 这里我们用 m_file 和 clean_count 来反推注入 demo 数确保 inject_rate 近似
