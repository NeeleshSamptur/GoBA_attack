"""
GoBA: action→(image+text) weight + QKTV across a T-step rollout.
One demo per task (seed=7), clean vs poison.
Saves per-step FTT and mean-DoF image attention maps for visualization.
"""
import json
import os
import sys
...
def main():
    ...
    for task_id in range(min(MAX_TASKS, task_suite.n_tasks)):
        ...
        for cond, bddl in BDDL.items():
            ...
            env.close()
            T = len(ep["ftt_w"])
            print(f"GoBA task={task_id} {cond:6s} T={T}  "
    ...
    print(f"Saved {n} episodes -> {OUT}/goba_temporal.npz", flush=True)
