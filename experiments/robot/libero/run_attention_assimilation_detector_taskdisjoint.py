...
OUT_NPZ = f"{REPO}/attn_maps/assimilation_stats_alllayers_fnorm_taskdisjoint.npz"
...
SEEDS = [7, 42, 1234, 2026, 31337, 5, 99, 777, 20260803, 424242]
CLEAN_TASKS = [0, 1, 2, 3, 4]
POISON_TASKS = [5, 6, 7, 8, 9]
NUM_STEPS_WAIT = 10
DEVICE = 0
...
def main():
    ...
    task_cond_pairs = [(t, "clean") for t in CLEAN_TASKS] + [(t, "poison") for t in POISON_TASKS]
    for task_id, cond in task_cond_pairs:
        task = task_suite.get_task(task_id)
        bddl_dir = CONDITIONS[cond]
        if True:
            for seed in SEEDS:
